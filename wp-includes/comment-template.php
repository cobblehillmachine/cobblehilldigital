...
<?php endif; ?>
		...
		<div class="comment-meta commentmetadata">
			<?php
				...
				printf( __('%1$s'), get_comment_date(),  get_comment_time()) ?><?php edit_comment_link(__('(Edit)'),'&nbsp;&nbsp;','' );
			?>
		</div>
...
function comment_form( $args = array(), $post_id = null ) {
	...
	$fields =  array(
		'author' => '<div class="input-wrap">' . '<label for="author">' . __( 'Name' ) . ( $req ? '<span class="required"> *</span></label>' : '' ) .
		            '<br /><input id="author" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30"' . $aria_req . ' /></div>',
		'email'  => '<div class="input-wrap"><label for="email">' . __( 'Email' ) . ( $req ? '<span class="required"> *</span></label> ' : '' ) .
		            '<br /><input id="email" name="email" type="text" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30"' . $aria_req . ' /></div>',
		'url'    => '<div class="input-wrap" style="margin-bottom: 20px;"><label for="url">' . __( 'Website' ) . '</label>' .
		            '<br /><input id="url" name="url" type="text" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30" /></div>',
	);

	$required_text = sprintf( ' ' . __('Required fields are marked %s'), '<span class="required">*</span>' );
	$defaults = array(
		'fields'               => apply_filters( 'comment_form_default_fields', $fields ),
		'comment_field'        => '<label>' . _x( 'Message', 'noun' ) . '</label>' . '<div class="textarea-wrap">' . '<textarea id="comment" name="comment" rows="8" aria-required="true"></textarea></div>',
